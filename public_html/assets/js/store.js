/* ============================================================
   ...
   Catálogo, sacola, checkout e acompanhamento via API PHP.
   Conta, pedidos e cupons ficam no servidor (sessão + hash).
   ============================================================ */
"use strict";

let savedCart = [];
try { savedCart = JSON.parse(localStorage.getItem("lapanini_cart") || "[]") || []; } catch { savedCart = []; }
const state = { menu: null, cart: savedCart, coupon: null, customer: null };

const API_BASE = (window.LAPANINI && window.LAPANINI.api) || "/api";
const csrf = () => (window.LAPANINI && window.LAPANINI.csrf) || qs('meta[name="csrf-token"]')?.content || "";
...
let COUPONS = { LAPANINI10: 0.1, QUARTA: 0.15 };
const WHATSAPP_NUMBER = (window.LAPANINI && window.LAPANINI.whatsapp) || "5519994048354";
...
async function bootFromServer() {
  try {
    ...
    if (session.user) {
      state.customer = session.user;
      MENU.customer = session.user;
    }
  } catch {}
  try {
    const payload = await api("/menu");
    if (payload.data) {
      Object.assign(MENU, payload.data);
      state.menu = MENU;
      const bar = qs("#categories");
      if (bar) bar.innerHTML = '<button class="active" data-category="all">Todos</button><button data-category="featured">Destaques</button>';
      renderCategories();
      renderRows();
      setupCategorySync();
    }
  ...
  try {
    const payload = await api("/cupons");
    if (payload.coupons) {
      COUPONS = Object.fromEntries(payload.coupons.map(c => [c.code, c.percent]));
    }
  } catch {}
}
...
function openCart() {
  ...
  const payments = [
    ...
    ["credit", "Crédito na entrega"],
      ...
      const customer = state.customer || state.menu.customer;
...
async function accountSubmit(e) {
  ...
  try {
    ...
    MENU.customer = data.user;
    toast(mode === "register" ? "Conta criada com sucesso" : "Login realizado");
    openCart();
  } catch (err) {
    toast(err.message || "Não foi possível autenticar");
  }
}
...
function openAccount() {
  ...
  m.hidden = false;
  document.body.classList.add("modal-open");
  const connected = state.customer || state.menu?.customer;
  ...
  qs("#modalAccount", m)?.addEventListener("submit", async e => {
    ...
    if (state.customer) openAccount();
  });
  qs("#modalAccountLogout", m)?.addEventListener("click", async () => {
    await accountLogout();
    openAccount();
  });
...
async function renderAccountHistory(m) {
  const history = qs(".account-history", m);
  if (!history) return;
  let orders = [];
  try {
    const payload = await api("/pedidos");
    orders = payload.orders || [];
  } catch {
    history.innerHTML = "<p>Entre na sua conta para ver os pedidos salvos no servidor.</p>";
    return;
  }
  history.innerHTML = `<h3>Últimos pedidos</h3>` +
    (orders.length
      ? orders.map((o, n) => `
        <article><div><b>#${esc(o.order_number)}</b><span>${new Date(o.created_at).toLocaleDateString("pt-BR")} · ${esc(statusLabel(o.status || orderStatus(o)))}</span></div><strong>${money(o.total)}</strong>
...
async function applyCoupon() {
  ...
  try {
    const payload = await api("/cupons", { method: "POST", body: { code } });
    state.coupon = payload.coupon;
    COUPONS[payload.coupon.code] = payload.coupon.percent;
    toast(`Cupom ${payload.coupon.code} aplicado: ${payload.coupon.percent * 100}% OFF`);
  } catch (err) {
    state.coupon = null;
    toast(err.message || "Cupom inválido ou expirado");
  }
  openCart();
}
...
async function checkout(e) {
  ...
  try {
    const payload = await api("/pedidos", { method: "POST", body: { ...f, items: state.cart, coupon_code: state.coupon?.code || f.coupon_code } });
    Object.assign(order, payload.order || {});
  } catch (err) {
    return toast(err.message || "Não foi possível gravar o pedido no servidor");
  }
...
function openTracker() {
  ...
  const load = async payload => {
    let order = null;
    try {
      const data = await api("/track", { method: "POST", body: { email: payload.email } });
      order = data.order;
    } catch (err) {
      qs("#trackResult", m).innerHTML = `<p class="track-error">${esc(err.message || "Nenhum pedido encontrado com esse e-mail.")}</p>`;
      return;
    }
    if (!order) {
    ...
    const status = order.status || orderStatus(order);
